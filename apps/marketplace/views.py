...
def public_service_queryset():
    return Service.objects.filter(
        status='active',
        provider__provider_profile__status='active',
...
def home_view(request):
    ...
    # إحصائيات
    stats = {
        'services_count': public_service_queryset().count(),
...
class ServiceCreateView(LoginRequiredMixin, UserPassesTestMixin, CreateView):
    ...
    def get_form_kwargs(self):
        ...
        kwargs['provider_user'] = self.request.user
        return kwargs
...
def my_services_view(request):
    ...
    total_orders = sum(s.real_orders for s in services)
