...
class ProviderServiceDashboardRenderingTests(DashboardAccessTests):
    def test_provider_services_page_renders_catalog_only_rows_without_none_title_error(self):
        self.client.login(username='admin', password='pass')
        response = self.client.get(reverse('dashboard:provider_services'))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, self.managed.name)
        self.assertContains(response, 'اعتماد خدمة مركزية')
        self.assertNotContains(response, '{{ ps.service.title }}')

    def test_provider_services_page_renders_commercial_service_title_when_present(self):
        self.client.login(username='admin', password='pass')
        self.client.get(reverse('dashboard:provider_services'))
        response = self.client.get(reverse('dashboard:provider_services'), {'q': self.service.title})
        self.assertEqual(response.status_code, 200)
