...
def service_action(request, obj, action, reason=''):
    ...
    if isinstance(obj, Service):
        if action in {'activate','publish'}:
            if not obj.provider_service_id:
                raise ValidationError('لا يمكن نشر خدمة غير مرتبطة باعتماد خدمة مركزية.')
            if not obj.provider_service.is_active or obj.provider_service.approval_status not in {'approved', 'active'}:
                raise ValidationError('لا يمكن نشر خدمة دون اعتماد مقدم خدمة نشط ومعتمد.')
            obj.status='active'
