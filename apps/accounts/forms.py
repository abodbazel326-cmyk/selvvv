...
class UserRegisterForm(LocationFieldsMixin, UserCreationForm):
    ...
    """
    first_name = forms.CharField(label='الاسم الأول', required=True, widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'الاسم الأول'}))
    last_name = forms.CharField(label='اسم العائلة', required=True, widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'اسم العائلة'}))
...
class UserProfileForm(LocationFieldsMixin, forms.ModelForm):
    ...
    def __init__(self, *args, include_location=True, partial=False, **kwargs):
        super().__init__(*args, **kwargs)
        if partial:
            for field in self.fields.values():
                field.required = False
        if not include_location:
            self.fields.pop('location_city', None)
            self.fields.pop('location_district', None)
        else:
...
class ProviderProfileForm(LocationFieldsMixin, forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'qualification_choices': forms.SelectMultiple(attrs={'class': 'form-select choices-dropdown', 'size': 1, 'title': 'يمكن اختيار أكثر من مؤهل'}),
    ...
    def __init__(self, *args, partial=False, **kwargs):
        ...
        # These model defaults should not turn a partial onboarding submission
        # into a validation failure when the provider has not reached that step.
        self.fields['experience_years'].required = False
        self.fields['service_radius'].required = False

    def clean_experience_years(self):
        value = self.cleaned_data.get('experience_years')
        return self.instance.experience_years if value is None else value

    def clean_service_radius(self):
        value = self.cleaned_data.get('service_radius')
        return self.instance.service_radius if value is None else value
...
class ProviderVerificationRequestForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {'requested_services': forms.SelectMultiple(attrs={'class': 'form-select choices-dropdown', 'size': 1, 'form': 'provider-onboarding-form', 'title': 'يمكن اختيار أكثر من خدمة'})}
    ...
    def clean_requested_services(self):
        services = self.cleaned_data.get('requested_services')
        return services
