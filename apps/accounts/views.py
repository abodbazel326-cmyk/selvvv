...
def register_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user.is_provider():
                services.get_provider_profile(user)
                messages.success(request, 'تم إنشاء حسابك بنجاح. ابدأ الآن توثيق وتدشين حساب مقدم الخدمة، ويمكنك حفظ التقدم كمسودة قبل الإرسال للإدارة.')
                return redirect('accounts:provider_onboarding')
...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user.is_provider() and not services.get_provider_profile(user).verification_status == 'verified':
                return redirect('accounts:provider_onboarding')
...
def _save_provider_forms(request, profile, include_wallets=False):
    """Save the canonical user/profile data used by both independent workflows."""
    user_form = UserProfileForm(request.POST, instance=request.user, prefix='user', include_location=False, partial=True)
    provider_form = ProviderProfileForm(request.POST, request.FILES, instance=profile, prefix='provider', partial=True)
    if not (user_form.is_valid() and provider_form.is_valid()):
        return False, user_form, provider_form, None
    ...
    try:
        ...
        return False, user_form, provider_form, exc
    return True, user_form, provider_form, None


@login_required
def provider_profile_edit_view(request):
    """Profile-only editor; verification actions are intentionally not handled here."""
    ...
    if request.method == 'POST':
        ...
        if save_error:
            messages.error(request, 'تعذر حفظ بيانات الملف الشخصي. تحقق من الحقول المدخلة.')
        else:
            messages.error(request, 'تعذر حفظ بيانات الملف الشخصي. راجع أخطاء الحقول أدناه.')
    else:
        user_form = UserProfileForm(instance=request.user, prefix='user', include_location=False)
    ...
    return render(request, 'accounts/provider_profile_edit.html', {
        ...
        'profile': profile,
        'approved_provider_services': approved_provider_services,
...
def provider_onboarding_view(request):
    """Independent provider verification/onboarding workflow using existing models."""
    if not request.user.is_provider():
        messages.error(request, 'هذه الصفحة متاحة لمقدمي الخدمات فقط.')
        return redirect('accounts:profile')
    ...
    user_form = UserProfileForm(instance=request.user, prefix='user', include_location=False, partial=True)
    provider_form = ProviderProfileForm(instance=profile, prefix='provider', partial=True)
    document_form = ProviderDocumentForm()

    # Keep the last reached step in the session as well as in the URL, so
    # returning to onboarding never unexpectedly starts from step 1.
    requested_step = request.GET.get('step')
    posted_step = request.POST.get('wizard_step')
    saved_step = request.session.get('provider_onboarding_step', 1)
    try:
        current_step = max(1, min(8, int(requested_step or posted_step or saved_step or 1)))
    ...
    if request.method == 'POST':
        ...
        try:
            current_step = max(1, min(8, int(request.POST.get('wizard_step', current_step) or current_step)))
        except (TypeError, ValueError):
            current_step = 1
        request.session['provider_onboarding_step'] = current_step
        if action == 'accept_commission':
            ...
            if not terms:
                ...
                messages.success(request, 'تم حفظ البيانات والموافقة على سياسة العمولة بنجاح.')
                request.session['provider_onboarding_step'] = 5
                return redirect(f'{request.path}?step=5')
        elif action == 'upload_document':
            # Document uploads use their own multipart form. Profile data is already
            # autosaved before entering this step, so do not re-bind an empty profile
            # form here (that could clear fields or many-to-many selections).
            ...
            if document_form.is_valid():
                ...
                messages.success(request, 'تم رفع المستند وحفظ بيانات التوثيق الحالية.')
                request.session['provider_onboarding_step'] = 6
                return redirect(f'{request.path}?step=6')
            messages.error(request, 'تعذر رفع المستند. تحقق من النوع والحجم.')
        elif action == 'delete_document':
            document = get_object_or_404(ProviderDocument, pk=request.POST.get('document_id'), provider=profile)
            if document.status in {'pending', 'rejected', 'needs_additional_documents'}:
            ...
            # The submit action saves the latest form state first, then validates
            # the complete onboarding checklist and creates the existing request.
            ...
            if not saved:
                if save_error:
                    ...
                    messages.error(request, 'تعذر حفظ بيانات التوثيق. راجع أخطاء الحقول أدناه.')
            else:
                profile.refresh_from_db()
                submitted, message, verification_form = _validate_and_submit_verification(request, profile)
                if submitted:
            ...
            if save_error:
                ...
                messages.error(request, 'تعذر حفظ التغييرات. راجع أخطاء الحقول أدناه.')

    request._provider_onboarding_step = current_step
    return render(request, 'accounts/provider_onboarding.html', _provider_onboarding_context(
        request, profile, user_form, provider_form, document_form,
        locals().get('verification_form'),
    ))
...
def _provider_onboarding_context(request, profile, user_form, provider_form, document_form, verification_form=None):
    ...
    return {
        ...
        'current_step': getattr(request, '_provider_onboarding_step', 1),
    }
...
def accept_commission_policy(request):
    ...
    return redirect(f"{reverse('accounts:provider_onboarding')}?step=5")
...
def _initial_requested_services(profile):
...
def _create_or_update_verification_request(request, profile, requested_services):
    """Persist a submitted verification request using the existing request model."""
    with transaction.atomic():
        verification = profile.verification_requests.exclude(status='approved').order_by('-created_at').first()
        if verification is None:
            verification = ProviderVerificationRequest(provider=profile)
        ...
        verification.save()
        verification.requested_services.set(requested_services)
        ...
        profile.save(update_fields=['verification_status', 'status', 'updated_at'])

    from apps.core.services import notify
    for admin in User.objects.filter(is_staff=True):
        notify(admin, 'provider_submitted', 'طلب توثيق جديد', f'{request.user.username} أرسل حسابه للمراجعة')
    return verification


def _validate_and_submit_verification(request, profile):
    """Submit the current onboarding draft even when optional sections are incomplete."""
    ...
    missing = {
        ...
        'experience': 'الخبرة والمؤهلات',
        'documents': 'المستندات المطلوبة',
        'location': 'الموقع المهني',
        'terms': 'الموافقة على الشروط والعمولة',
        'wallet': 'بيانات المحافظ',
    }
...
def provider_submit_review(request):
    """Backward-compatible review endpoint; the onboarding page is the canonical caller."""
    if not request.user.is_provider():
        messages.error(request, 'هذه الصفحة لمقدمي الخدمات فقط.')
        return redirect('home')
    profile = services.get_provider_profile(request.user)
    if profile.verification_status == 'verified':
        messages.info(request, 'حسابك موثق بالفعل.')
        return redirect('accounts:provider_profile_edit')
    if request.method != 'POST':
        return redirect('accounts:provider_onboarding')

    submitted, message, _ = _validate_and_submit_verification(request, profile)
    if submitted:
        messages.success(request, message)
    else:
        messages.error(request, message)
